...
class TestQuestionListFilterSearch:
    ...
    def questions(self, user, tags):
        ...
        q3 = Question.objects.create(
            owner=user, title="General question", body="Tell me about yourself"
        )
    ...
    def test_filter_by_tag(self, client, user, questions, tags):
        ...
        # selected_tag is the slug (lowercase), selected_tag_name is the
        # display name
    ...
    def test_search_in_question_title(self, client, user, questions):
        ...
        client.force_login(user)
        response = client.get(
            reverse("questions:list"), {"search": "leadership"}
        )
    ...
    def test_pagination_preserves_filters(self, client, user, tags):
        ...
        for i in range(15):
            q = Question.objects.create(
                owner=user, title=f"Question {i}", body="Test question"
            )
            if i % 2 == 0:
                q.tags.add(tags["public"])

        response = client.get(
            reverse("questions:list"), {"tag": "Leadership", "page": "1"}
        )
    ...
    def test_no_results_shows_zero_message(self, client, user):
        """
        Test that when search/filter returns no results, it shows
        '0 Questions found'.
        """
        client.force_login(user)
        ...
        response = client.get(
            reverse("questions:list"), {"search": "zzzznonexistent"}
        )
