...
class QuestionEditViewTest(TestCase):
    ...
    def test_edit_private_question_get(self):
        ...
        url = reverse(
            "questions:edit", kwargs={"pk": self.private_question.pk}
        )
        ...
        self.assertContains(
            response, "border-dashed border-accent"
        )  # Edit styling
    ...
    def test_edit_private_question_post_success(self):
        ...
        self.assertEqual(
            self.private_question.body, "Updated question body content."
        )
        self.assertFalse(self.private_question.is_public)

    def test_private_question_always_has_approved_status(self):
        """
        Test that private questions always have STATUS_APPROVED
        after editing.
        """
        self.client.login(username="testuser", password="testpass123")
        url = reverse(
        ...
        self.private_question.refresh_from_db()
        self.assertEqual(
            self.private_question.status, Question.STATUS_APPROVED
        )
        self.assertFalse(self.private_question.is_public)

    def test_edit_public_question_forbidden(self):
        """
        Test that non-admin users editing public questions reverts
        status to PENDING.
        """
        self.client.login(username="testuser", password="testpass123")
        ...
        self.assertEqual(
            self.public_question.title, "Updated Public Question Title"
        )
    ...
    def test_admin_can_edit_other_users_questions(self):
        ...
        self.private_question.refresh_from_db()
        self.assertEqual(
            self.private_question.title, "Admin Edited Private Question"
        )
    ...
    def test_admin_can_delete_any_question(self):
        ...
        url = reverse(
            "questions:delete", kwargs={"pk": self.public_question.pk}
        )
        response = self.client.post(url)

        self.assertEqual(response.status_code, 302)
        self.assertFalse(
            Question.objects.filter(pk=self.public_question.pk).exists()
        )
    ...
    def test_non_admin_cannot_delete_other_users_questions(self):
        """
        Test that non-admin users cannot delete questions owned
        by others.
        """
        self.client.login(username="otheruser", password="otherpass123")
