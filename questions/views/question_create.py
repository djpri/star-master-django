...
def question_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if request.user.is_superuser:
                ...
                approval_message = (
                    "Your question is pending review by a moderator."
                )
            else:
                question.status = Question.STATUS_APPROVED
                approval_message = (
    ...
    is_public_question = (
        force_public
        or form_data_public in truthy_values
        or form.initial.get("is_public") is True
    )

    # Get available tags depending on visibility
    if is_public_question:
        # Only show public tags when creating a public question
