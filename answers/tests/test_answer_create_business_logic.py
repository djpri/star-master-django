...
class AnswerCreateBusinessLogicTest(TestCase):
    ...
    def test_can_create_answer_for_private_question(self):
        ...
        url = reverse(
            "answers:create", kwargs={"question_id": self.private_question.pk}
        )
    ...
    def test_can_create_answer_for_public_pending_question(self):
        """
        Users can create answers for public questions that are
        still pending approval
        """
        self.client.login(username="testuser", password="testpass123")
        url = reverse(
            "answers:create",
            kwargs={"question_id": self.public_pending_question.pk},
        )
    ...
    def test_can_create_answer_for_public_denied_question(self):
        ...
        url = reverse(
            "answers:create",
            kwargs={"question_id": self.public_denied_question.pk},
        )
    ...
    def test_cannot_create_answer_for_public_approved_question(self):
        ...
        url = reverse(
            "answers:create",
            kwargs={"question_id": self.public_approved_question.pk},
        )
        ...
        self.assertContains(
            response,
            "Public questions are designed to be used as read-only examples",
            status_code=404,
        )
