...
class AnswerDeleteViewTest(TestCase):
    ...
    def test_owner_can_delete_star_answer(self):
        ...
        self.assertEqual(response.url, reverse(
            'questions:detail', kwargs={'pk': self.question.pk}))

        # Answer should be deleted
        self.assertFalse(Answer.objects.filter(
            pk=self.star_answer.pk).exists())
        self.assertFalse(StarAnswer.objects.filter(
            pk=self.star_answer.pk).exists())
    ...
    def test_owner_can_delete_basic_answer(self):
        ...
        # Answer should be deleted
        self.assertFalse(Answer.objects.filter(
            pk=self.basic_answer.pk).exists())
        self.assertFalse(BasicAnswer.objects.filter(
            pk=self.basic_answer.pk).exists())
    ...
    def test_non_owner_cannot_delete_answer(self):
        ...
        self.assertTrue(Answer.objects.filter(
            pk=self.other_answer.pk).exists())
